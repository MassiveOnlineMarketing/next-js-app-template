...
export class GoogleSearchKeyword {
  id: string;
  keyword: string;
  googleSearchCampaignId: string;
  createdAt: Date;
  ...
  static createAsPlainObject(
    ...
    const keywordInstance = new GoogleSearchKeyword(id, keyword, googleSearchCampaignId, createdAt);
  ...
  static generateAndFormatKeywords(projectId: string, keywordNames: string[]) {
    ...
    const keywords = keywordNames.map(keywordName => 
      GoogleSearchKeyword.createAsPlainObject(
