...
export async function fetchGoogleSearchConsoleKeywordDetailsData(keywordName: string, websiteId: string) {
  ...
  try {
    const googleSearchConsoleData = await googleSearchConsoleApiService.fetchKeywordDetailsData(keywordName, websiteId);
